...
## Queueing Mechanism
For managing the chat flow and message processing, a robust queuing mechanism will be employed. Django Q (using `qcluster` for workers) will be used for this purpose. This system is crucial for asynchronous processing, especially for long-running tasks like interacting with the OpenAI API, and ensuring scalability and reliability of message handling as outlined in `documents/FULL_PLAN.md` under "Message Processing Flow (Detailed Steps)". Documentation for its specific usage will be provided alongside its implementation.

### Qcluster Logging
For easier debugging and monitoring of asynchronous tasks, ensure that `qcluster` is configured to output its logs to a centralized and accessible location. This typically involves proper Django logging configuration to capture logs from the `django_q` application.
...
## Planning Workflow
When initiating a new task, especially one requiring a detailed plan, the following workflow will be strictly adhered to:
1.  **Collaborative Planning:** The planning phase will be a thorough, collaborative process. The agent will ask for every necessary detail to fully understand the task, including its purpose, technical specifics, and desired outcomes.
2.  **Comprehensive Outline:** All gathered input will be organized into a comprehensive outline. This outline must be agreed upon as complete and polished by both the user and the agent before proceeding with any implementation steps.
3.  **No Guesswork:** The goal of this planning is to eliminate guesswork entirely. Every aspect of the task should be clearly defined.
4.  **Test-Driven Development (TDD):** All code changes will be developed following TDD principles.
5.  **Pre-Edit Code Review:** Before making any edits, the relevant code will be re-read to ensure a complete understanding of its current state and context.
6.  **Granular Deletions:** When modifying code, deletions exceeding 30 lines will be avoided. If large sections need to be removed, this will be done through a series of smaller, atomic deletions.
7.  **Documentation Updates:** The `documents/FULL_PLAN.md` file, and any other relevant documentation, will be updated to reflect adjustments made during the task, if necessary.
8.  **Delayed TODO Creation:** Task-specific TODO lists will only be created once the planning phase is 100% complete and approved.
9.  **Proactive Code Review:** The agent will proactively read relevant project code during the planning phase to gain a better understanding and improve the quality of the plan.